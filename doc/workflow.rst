...
gas connection, but the pipeline can only deliver 50 kW worth of Gas power.
...
parks up to 200 kW, only surpased by the great offshore sites of *Jepid Island*
with 250 kW potential capacity. The other islands only have space for up to
120 kW or 80 kW. *Gas plants* can be built up to 100 kW on every island, except
for *Vled Haven*, which can house up to 80 kW only. 
...
to 9 M$/kWh. This yields the following table:
...
population centers of each island. So *Jepid Island* with 1.1 M$/kW investment
costs is more than twice as far away from *Vled Haven* than *Ylyph Archipelago*
with only 0.5 M$/kW. *Stryworf Key* is somewhere between with 0.8 M$/kW. All
...
series. *Vled Haven* has the highest peak load with 75 kW, followed by 
*Stryworf Key* with 19 kW and the other islands with 8.2 kW each:
...
That's what :func:`save` is made for:
    ...
    >>> MvsoModel.save(prob, 'newsealand-base.pgz')
...
result numbers, use :func:`report`::
    ...
    >>> MvsoModel.report(prob, 'report.xlsx',prob.com_demand,prob.sit)
...
you are only interested in *electricity* in *Vled Haven*::
    ...
    >>> MvsoModel.report(prob, 'report-vled-haven.xlsx',
    ...             ['Elec'], ['Vled Haven'])
...
that you use :func:`plot`::
    ...
    >>> %matplotlib
    >>> fig = MvsoModel.plot(prob, 'Elec', 'Vled Haven')
...
.. _installation instructions: 
   https://github.com/tum-ens/MvsoModel/blob/master/README.md#installation
.. _get started:
   https://github.com/tum-ens/MvsoModel/blob/master/README.md#get-started
