.. MvsoModel documentation master file, created by
...
MvsoModel: A linear optimisation model for distributed energy systems
================================================================
...
Features
--------
* `MvsoModel`_ is a linear programming model for multi-commodity energy systems with 
...
* The model itself is quite small thanks to relying on the `Pyomo`_
  package.
* MvsoModel includes reporting and plotting functions for rapid scenario 
  development.
...
* Maintenance: Model file ``MvsoModel.py`` split into subfiles in folder ``MvsoModel``
...
* As a consequence :func:`plot` now plots commodity balance by processes, not 
  input commodities.
* MvsoModel now supports input files with only a single site; simply delete all
...
* More docstrings and comments in the main file ``MvsoModel.py``.


Screenshots
-----------

This is a typical result plot created by :func:`MvsoModel.plot`, showing electricity
...
.. _readme.md: https://github.com/tum-ens/MvsoModel/blob/master/README.md#installation
.. _MvsoModel: https://github.com/tum-ens/MvsoModel
